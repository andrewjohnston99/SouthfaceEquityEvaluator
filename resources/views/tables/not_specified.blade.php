<tr class="text-center">
    ...
    <td class="align-middle">
        <div class="pretty p-default p-thick p-smooth">
            <input id="option-{{ $question->options->first()->id }}" name="option-{{ $question->options->first()->id }}" type="checkbox" data-value="{{ $question->options->first()->points }}" {{ (isset($data['answers']) && array_search($question->options->first()->id, array_column($data['answers']->toArray(), 'option_id')) !== false) ? 'checked="checked"' : '' }} />
